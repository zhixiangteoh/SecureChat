...
/*
Simple secure chat server with RSA encryption envelope system, 
with Add128 and Substitution symmetric chat ciphers.
...
public class SecureChatServer {
    ...
    private ArrayList<Socket> users; 	   // one per client
    private ArrayList<UserThread> threads; // one per client
    ...
    public SecureChatServer() throws IOException
    {
        ...
        threads = new ArrayList<UserThread>();   // Set things up and start
            ...
            // it to encode the message, and send the result.
    {
        for (int i = 0; i < numUsers; i++)
        {
            ...
            ObjectOutputStream currWriter = threads.get(i).getWriter();
			SymCipher currCipher = threads.get(i).getCipher();
    ...
    public synchronized void removeClient(int id, String name)
    {
        ...
        {                            // also must be synchronized, since we
            users.get(id).close();   // could have an inconsistent state if this
			...
			while (true)
			{
				try
				{
					...
					synchronized (this)
					{
						...
						threads.add(new UserThread(newSocket, numUsers,
													newName, tempReader, tempWriter, cipher));
						threads.get(numUsers).start();
						// Above a new thread is created and started for the
						// new user

						System.out.println("Connection " + numUsers + users.get(numUsers));
						numUsers++;
					}
				}
    ...
    {
         SecureChatServer Secure = new SecureChatServer();
    }
}